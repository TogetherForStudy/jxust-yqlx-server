...
func (s *CourseTableService) GetUserCourseTable(userID uint, semester string) (*response.CourseTableResponse, error) {
	return s.GetUserCourseTableWithVersion(userID, semester, nil)
}

// GetUserCourseTableWithVersion 获取用户课程表（带版本检测）
func (s *CourseTableService) GetUserCourseTableWithVersion(userID uint, semester string, clientLastModified *int64) (*response.CourseTableResponse, error) {
	// 先获取用户信息，获取其班级ID
