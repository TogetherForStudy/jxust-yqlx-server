...
func buildOperations() []operation {
	...
	ops = append(ops,
		...
		op("GET", "/api/v0/admin/stats/projects/online", "AdminStats", "获取所有启用项目在线人数",
			withSecurity(constant.PermissionStatisticManage),
			withEnvelopeType[resp.AllProjectsOnlineStatResponse](),
		),
		...
		op("GET", "/api/v0/organizations/", "Organizations", "获取组织列表",
			withSecurity(constant.PermissionOrganizationGet),
			withQueryType[req.ListOrganizationsRequest](),
			withEnvelopeResponse(pageSchema(typeSchema[models.Organization]())),
		),
		op("GET", "/api/v0/organizations/{id}", "Organizations", "获取组织详情",
			withSecurity(constant.PermissionOrganizationGet),
			withParams(pathIntParam("id", "组织 ID")),
			withEnvelopeType[models.Organization](),
		),
		op("GET", "/api/v0/admin/organizations", "Organizations", "管理员获取组织列表",
			withSecurity(constant.PermissionOrganizationManage),
			withQueryType[req.ListOrganizationsRequest](),
			withEnvelopeResponse(pageSchema(typeSchema[models.Organization]())),
		),
		op("GET", "/api/v0/admin/organizations/{id}", "Organizations", "管理员获取组织详情",
			withSecurity(constant.PermissionOrganizationManage),
			withParams(pathIntParam("id", "组织 ID")),
			withEnvelopeType[models.Organization](),
		),
		op("POST", "/api/v0/admin/organizations", "Organizations", "管理员创建组织",
			withSecurity(constant.PermissionOrganizationManage),
			withIdempotency(),
			withJSONBodyType[req.CreateOrganizationRequest](),
			withEnvelopeType[models.Organization](),
		),
		op("PUT", "/api/v0/admin/organizations/{id}", "Organizations", "管理员更新组织",
			withSecurity(constant.PermissionOrganizationManage),
			withParams(pathIntParam("id", "组织 ID")),
			withJSONBodyType[req.UpdateOrganizationRequest](),
			withEnvelopeType[models.Organization](),
		),
		op("DELETE", "/api/v0/admin/organizations/{id}", "Organizations", "管理员删除组织",
			withSecurity(constant.PermissionOrganizationManage),
			withParams(pathIntParam("id", "组织 ID")),
			withEnvelopeResponse(messageSchema()),
		),
